class Logger {
  ...
  log(level, type, logData) {
    const labels = { component: this.config.logs.source, level: level, type: type };
  ...
  async sendLogToGrafana(event) {
    ...
    try {
      const res = await fetch(`${this.config.logs.url}`, {
        method: 'post',
        body: body,
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${this.config.logs.userId}:${this.config.logs.apiKey}`,
        },
      });
      if (!res.ok) {
